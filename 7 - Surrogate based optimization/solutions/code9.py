...
for k in range(n_iter):
    ...
    gpr_con.train()
    con = ({'type': 'ineq', 'fun': lambda x: gpr_con.predict_values(x)[:,0]})
    ...
    opt_all = np.array([minimize(obj_k, x_st, method='SLSQP', bounds=[(0,25)], constraints=con) for x_st in x_start])
    ...
    y_data_con = np.atleast_2d(np.append(y_data_con,y_con_et_k)).T
